#!/usr/bin/env python3
"""Eval test: Web — Flask + HTMX Shop. Run: cd smoke-tests && python3 80-web-htmx/smoke-eval.py"""
...
SVC="80-web-htmx"; PORT=16080; FIXTURE_DIR=os.path.join(os.path.dirname(__file__),"fixtures","blank-htmx-shop")
...
print(f"\n{'='*62}\nEDOT-Autopilot | {SVC}\n{'='*62}")
print(f"  Fixture: blank-htmx-shop (Flask + HTMX, no OTel)  |  port: {PORT}\n")
...
tmpdir=tempfile.mkdtemp(prefix="edot-autopilot-htmx-")
proc=None
try:
    ...
    subprocess.run(["git","commit","-m","initial: blank Flask+HTMX app, no observability"],cwd=tmpdir,capture_output=True,check=True)
    ...
    if pip.returncode==0:
        ...
        if alive:
            try:
                resp=urllib.request.urlopen(f"http://localhost:{PORT}/products",timeout=5)
                check("GET /products returns 200",resp.status==200,f"status={resp.status}")
            except Exception as e: check("GET /products returns 200",False,str(e))
    else:
        check("App starts and /health responds",False,"pip install failed")
        check("GET /products returns 200",False,"app did not start")
