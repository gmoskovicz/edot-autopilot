#!/usr/bin/env python3
"""Eval test: Web — Go + Gin API. Run: cd smoke-tests && python3 77-web-gin-go/smoke-eval.py"""
...
SVC="77-web-gin-go"; FIXTURE_DIR=os.path.join(os.path.dirname(__file__),"fixtures","blank-gin-api")
...
print(f"  Fixture: blank-gin-api (Go + Gin, no OTel)  |  go: {'found' if go_bin else 'not found'}\n")
...
tmpdir=tempfile.mkdtemp(prefix="edot-autopilot-gin-")
try:
    ...
    subprocess.run(["git","commit","-m","initial: blank Gin API, no observability"],cwd=tmpdir,capture_output=True,check=True)
    ...
    gomod=open(os.path.join(tmpdir,"go.mod")).read() if os.path.exists(os.path.join(tmpdir,"go.mod")) else ""
    all_content=gomod
    for root,_,files in os.walk(tmpdir):
        for f in files:
            if f.endswith('.go') and '.git' not in root: all_content+=open(os.path.join(root,f)).read()
    check("OTel added to go.mod","go.opentelemetry.io/otel" in gomod,gomod[:300])
    check("OTLP exporter referenced","otlptracehttp" in all_content or "otlptracegrpc" in all_content or "otlp" in all_content.lower(),"no OTLP exporter")
    check("TracerProvider initialized","TracerProvider" in all_content or "NewTracerProvider" in all_content,"no TracerProvider")
    ...
    if go_bin:
        go_build=subprocess.run([go_bin,"build","./..."],cwd=tmpdir,capture_output=True,text=True,timeout=120)
        check("go build passes",go_build.returncode==0,go_build.stderr[-200:] if go_build.returncode!=0 else "")
    else:
        check("go build passes",True,"# SKIP: go not found")
    check("Live app run",True,"# SKIP: requires Go runtime")
