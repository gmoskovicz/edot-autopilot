#!/usr/bin/env python3
"""
Eval test: Auth & Identity Platform (API gateway core service)
===============================================================
Runs `claude -p "Observe this project."` on the blank API gateway Flask service
(fixtures/blank-api-gateway/) and verifies the agent adds correct OTel instrumentation.

The API gateway is the CORE service of this 7-service auth platform.

Run:
    cd smoke-tests && python3 61-auth-platform/smoke.py
"""
...
SVC         = "61-auth-platform"
FIXTURE_DIR = os.path.join(os.path.dirname(__file__), "fixtures", "blank-api-gateway")
...
print(f"{'='*62}")
print(f"  Fixture: blank-api-gateway (Auth platform, Flask, no OTel)")
print(f"  Services modeled: api-gateway + auth + user-directory + mfa + token + session + audit")
print()
...
tmpdir = tempfile.mkdtemp(prefix="edot-autopilot-auth-")
try:
    ...
    subprocess.run(["git", "commit", "-m", "initial: blank auth API gateway, no observability"],
    ...
    check("Flask instrumentation added",
          "opentelemetry-instrumentation-flask" in req_content, req_content)
    check("OTLP exporter added",
          "opentelemetry-exporter-otlp" in req_content, req_content)
    ...
    if pip_result.returncode == 0:
        ...
        if started:
            try:
                ...
                check("POST /api/v1/login returns 200 or auth error",
                      r2.status in (200, 202, 401, 423),
                      f"status={r2.status}")
            except urllib.error.HTTPError as e:
                check("POST /api/v1/login returns 200 or auth error",
                      e.code in (200, 202, 401, 423), f"HTTP {e.code}")
            except Exception as e:
                check("POST /api/v1/login returns 200 or auth error", False, str(e))
...
print(f"  Kibana → APM → api-gateway (auth-platform)")
if failed:
    sys.exit(1)
