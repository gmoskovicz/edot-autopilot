#!/usr/bin/env python3
"""
E2E "Observe this project." — Tier C: psycopg2
===============================================
Runs `claude -p "Observe this project."` on a blank analytics ETL script
that uses psycopg2 to query and write to a PostgreSQL reporting schema.

EDOT Autopilot workflow:
  1. Reads blank fixture — finds psycopg2.connect / cursor.execute / fetchall
  2. Assigns Tier C (monkey-patch) — no official opentelemetry-instrumentation-psycopg2
  3. Wraps cursor.execute with CLIENT spans
  4. Adds business attributes: db.system.name=postgresql, db.operation.name, db.query.text
...
SVC         = "26-tier-c-psycopg2"
FIXTURE_DIR = os.path.join(os.path.dirname(__file__), "fixtures", "blank-psycopg2")
...
print(f"{'='*62}")
print(f"  Fixture: blank-psycopg2 (no OTel)")
...
tmpdir = tempfile.mkdtemp(prefix="edot-autopilot-psycopg2-")
try:
    ...
    check("Business attributes on spans",
          any(attr in app_content for attr in [
              "db.", "postgresql", "db.system", "db.operation", "db.query",
          ]),
          "no db.* attributes found in app.py")
    check("cursor.execute is patched",
          "execute" in app_content and (
              "_orig_" in app_content or "wrap" in app_content.lower()
          ),
          "cursor.execute patch not detected")
