#!/usr/bin/env python3
"""
Eval test: B2B SaaS Ops — Tenant Portal (core service)
========================================================
Runs `claude -p "Observe this project."` on the blank tenant-portal Flask service
(fixtures/blank-tenant-portal/) and verifies the agent adds correct OTel instrumentation.

Services modeled: tenant-portal + billing + provisioner + dns-manager + notification + auditor

Run:
    cd smoke-tests && python3 64-saas-ops/smoke.py
"""
...
SVC         = "64-saas-ops"
FIXTURE_DIR = os.path.join(os.path.dirname(__file__), "fixtures", "blank-tenant-portal")
...
print(f"{'='*62}")
print(f"  Fixture: blank-tenant-portal (SaaS provisioning, Flask, no OTel)")
print()
...
tmpdir = tempfile.mkdtemp(prefix="edot-autopilot-saas-")
try:
    ...
    subprocess.run(["git", "commit", "-m", "initial: blank tenant portal, no observability"],
    ...
    print("\nCode correctness checks (SaaS tenant provisioning instrumentation):")
        ...
        if started:
            try:
                ...
                r2 = urllib.request.urlopen(
                    ...
                    ), timeout=15  # provisioning takes time
                )
                check("POST /tenants returns 201 or error",
                      r2.status in (201, 402, 503), f"status={r2.status}")
            except urllib.error.HTTPError as e:
                check("POST /tenants returns 201 or error",
                      e.code in (201, 402, 503), f"HTTP {e.code}")
            except Exception as e:
                check("POST /tenants returns 201 or error", False, str(e))
...
print(f"  Kibana → APM → tenant-portal (saas-ops)")
if failed:
    sys.exit(1)
