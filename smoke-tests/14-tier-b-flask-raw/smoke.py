#!/usr/bin/env python3
"""
Eval: Tier B — Flask Without EDOT Middleware (Raw Flask + Manual Spans)
=======================================================================
Runs `claude -p "Observe this project."` on a blank Flask authentication
service that has no OTel instrumentation.

What this tests:
  1. The agent reads the blank app and understands it (Flask login API with
     MFA — validate credentials, send OTP via SMS/email/TOTP, create session).
  2. The agent correctly assigns Tier B (manual `with tracer.start_as_current_span(...)`
     wrapping, since the app is a legacy Flask version without an EDOT instrumentor
     or the user wants full per-handler control).
  3. The agent adds business enrichment attributes (user.*, auth.*, session.*).
  4. The agent uses record_exception on error paths.
  5. The generated requirements.txt includes the right OTel packages.
  6. The instrumented app starts and serves login requests correctly.
...
SVC         = "14-tier-b-flask-raw"
FIXTURE_DIR = os.path.join(os.path.dirname(__file__), "fixtures", "blank-flask-auth")
...
print(f"{'='*62}")
print(f"  Fixture: blank-flask-auth (no OTel)")
...
tmpdir = tempfile.mkdtemp(prefix="edot-autopilot-14-")
try:
    ...
    print("\nCode correctness checks (Tier B — manual Flask span wrapping):")
    ...
    check("Business enrichment attributes present",
          any(attr in app_content for attr in
              ["user.", "auth.", "session.", "customer.", "mfa."]),
          "no business enrichment attributes (user.*, auth.*, session.*) found")
    ...
    if started:
        ...
        # Valid login
        r_valid = http_lib.post(f"http://127.0.0.1:{PORT}/auth/login", json={
            "user_id": "USR-ENT-001", "password_hash": "hash_abc123", "mfa_channel": "sms",
        })
        check("POST /auth/login valid credentials -> 200",
              r_valid.status_code == 200,
              f"status={r_valid.status_code} body={r_valid.text[:200]}")
        if r_valid.status_code == 200:
            body = r_valid.json()
            check("Response has mfa_required=True", body.get("mfa_required") is True,
                  f"body: {body}")
            check("Response has session_id", "session_id" in body,
                  f"body: {body}")

        # Invalid login
        r_invalid = http_lib.post(f"http://127.0.0.1:{PORT}/auth/login", json={
            "user_id": "USR-PRO-042", "password_hash": "wrong_pass", "mfa_channel": "email",
        })
        check("POST /auth/login invalid credentials -> 401",
              r_invalid.status_code == 401,
              f"status={r_invalid.status_code} body={r_invalid.text[:200]}")
