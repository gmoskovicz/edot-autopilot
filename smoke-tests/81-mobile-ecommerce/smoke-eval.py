#!/usr/bin/env python3
"""Eval test: Mobile Ecommerce API. Run: cd smoke-tests && python3 81-mobile-ecommerce/smoke-eval.py"""
...
SVC="81-mobile-ecommerce"; PORT=16081; FIXTURE_DIR=os.path.join(os.path.dirname(__file__),"fixtures","blank-mobile-ecommerce-api")
...
print(f"\n{'='*62}\nEDOT-Autopilot | {SVC}\n{'='*62}")
print(f"  Fixture: blank-mobile-ecommerce-api (Flask mobile backend, no OTel)  |  port: {PORT}\n")
...
tmpdir=tempfile.mkdtemp(prefix="edot-autopilot-mobile-ecomm-")
proc=None
try:
    ...
    subprocess.run(["git","commit","-m","initial: blank mobile ecommerce API, no observability"],cwd=tmpdir,capture_output=True,check=True)
    ...
    check("FlaskInstrumentor or OTel init present","FlaskInstrumentor" in all_content or ("opentelemetry" in all_content.lower() and "instrument" in all_content.lower()),"no OTel init")
    ...
    if pip.returncode==0:
        proc=subprocess.Popen(["python3","app.py"],cwd=tmpdir,env=env,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
        alive=False
        for _ in range(15):
            time.sleep(1)
        ...
        if alive:
            try:
                resp=urllib.request.urlopen(f"http://localhost:{PORT}/api/v1/products",timeout=5)
                check("GET /api/v1/products returns 200",resp.status==200,f"status={resp.status}")
            except Exception as e: check("GET /api/v1/products returns 200",False,str(e))
    else:
        check("App starts and /health responds",False,"pip install failed")
        check("GET /api/v1/products returns 200",False,"app did not start")
