#!/usr/bin/env python3
"""
Eval: Tier B — Bottle Micro-Framework (No EDOT Instrumentor)
=============================================================
Runs `claude -p "Observe this project."` on a blank Bottle infrastructure
decommission API that has no OTel instrumentation.

What this tests:
  1. The agent reads the blank Bottle app and understands it (server
     decommission: dependency check, LB drain, archive).
  2. The agent correctly assigns Tier B (manual span wrapping — no official
     EDOT instrumentor exists for Bottle).
  3. The agent adds `with tracer.start_as_current_span(...)` with
     SpanKind.SERVER in each route function.
  4. The agent adds business enrichment attributes (server.*, infra.*,
     decommission.*).
  5. The agent uses record_exception on error paths.
  6. The instrumented app starts and handles decommission requests.
...
SVC         = "16-tier-b-bottle"
FIXTURE_DIR = os.path.join(os.path.dirname(__file__), "fixtures", "blank-bottle-infra")
...
print(f"{'='*62}")
print(f"  Fixture: blank-bottle-infra (no OTel)")
...
tmpdir = tempfile.mkdtemp(prefix="edot-autopilot-16-")
try:
    ...
    print("\nCode correctness checks (Tier B — manual Bottle span wrapping):")
    ...
    check("Business enrichment attributes present",
          any(attr in app_content for attr in
              ["server.", "infra.", "decommission.", "datacenter", "hostname"]),
          "no business enrichment attributes (server.*, infra.*, decommission.*) found")
    ...
    if started:
        ...
        for server in servers:
            r = http_lib.post(
                f"http://127.0.0.1:{PORT}/api/infra/servers/{server['hostname']}/decommission",
                json={"datacenter": server["datacenter"], "reason": server["reason"]},
                timeout=5,
            )
            check(
                f"POST /decommission {server['hostname']} {server['datacenter']} -> 200",
                r.status_code == 200,
            ...
            if r.status_code == 200:
                body = r.json()
                check(
                    f"archive_id has ARCH- prefix for {server['hostname']}",
                    str(body.get("archive_id", "")).startswith("ARCH-"),
                    f"archive_id: {body.get('archive_id')}",
                )
