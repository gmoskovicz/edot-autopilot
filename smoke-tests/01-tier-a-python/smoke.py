#!/usr/bin/env python3
"""
EVAL TEST: Tier A — FastAPI Fraud Detection (Real Agent Invocation)
===================================================================
*** EVAL TEST — runs `claude -p` and costs ~$1-2 per execution ***

This test ACTUALLY runs `claude -p "Observe this project."` on a blank,
uninstrumented FastAPI app. It does not assume or hardcode what the agent
will generate.

What this tests:
  1. The agent reads the blank app and understands it (FastAPI + fraud scoring
     + payment gateway + customer tiers)
  2. The agent correctly assigns Tier A (auto-instrumentation via
     opentelemetry-instrumentation-fastapi)
  3. The agent adds business enrichment (fraud.score, fraud.decision,
     order.total_usd, customer.tier, payment.status)
...
SVC = "01-eval-tier-a-fraud-detection"
FIXTURE_DIR = os.path.join(os.path.dirname(__file__), "fixtures", "blank-fraud-detection")
...
print(f"  Fixture: blank-fraud-detection (no OTel)")
...
tmpdir = tempfile.mkdtemp(prefix="edot-eval-01-")
try:
    ...
    print("\nBusiness enrichment checks:")
    has_fraud_enrichment = any(
        attr in app_content for attr in [
            "fraud.score", "fraud_score", "fraud.decision",
            "order.total", "customer.tier", "customer_tier",
            "payment.status",
        ]
    )
    check("Business span attributes added (fraud/order/customer/payment)",
          has_fraud_enrichment,
    ...
    if started:
        ...
        r_order = http_lib.post(f"http://127.0.0.1:{PORT}/orders", json={
            ...
            "items": [
                {"name": "Widget Pro", "price_usd": 299.99, "qty": 2},
                {"name": "Gadget Plus", "price_usd": 49.99, "qty": 1},
            ]
        })
