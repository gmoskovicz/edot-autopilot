#!/usr/bin/env python3
"""Eval test: Web — Vue 3 ShopClient. Run: cd smoke-tests && python3 73-web-vue/smoke-eval.py"""
...
SVC = "73-web-vue"
FIXTURE_DIR = os.path.join(os.path.dirname(__file__), "fixtures", "blank-vue-shop")
...
print(f"\n{'='*62}\nEDOT-Autopilot | {SVC}\n{'='*62}")
print(f"  Fixture: blank-vue-shop (Vue 3, no OTel)  |  NOTE: Live run SKIPPED\n")
claude_bin = shutil.which("claude")
check("claude CLI installed", claude_bin is not None)
check("CLAUDE.md exists", os.path.exists(CLAUDE_MD))
check("Fixture exists", os.path.isdir(FIXTURE_DIR))
...
tmpdir = tempfile.mkdtemp(prefix="edot-autopilot-vue-")
try:
    ...
    subprocess.run(["git","commit","-m","initial: blank Vue SPA, no observability"], cwd=tmpdir, capture_output=True, check=True)
    ...
    for root, _, files in os.walk(tmpdir):
        for f in files:
            if f.endswith(('.vue','.ts','.js')) and 'node_modules' not in root:
                all_content += open(os.path.join(root, f)).read()
    ...
    check("Elastic endpoint configured",
          "ELASTIC_OTLP_ENDPOINT" in all_content or "OTLP_ENDPOINT" in all_content or ENDPOINT.split("//")[-1][:20] in all_content, "endpoint not found")
    check("Live app run", True, "# SKIP: requires browser/bundler")
