#!/usr/bin/env python3
"""
Eval: Tier B — aiohttp Async Web Server (No EDOT Instrumentor)
===============================================================
Runs `claude -p "Observe this project."` on a blank aiohttp healthcare
appointment sync service that has no OTel instrumentation.

What this tests:
  1. The agent reads the blank aiohttp app and understands it (sync patient
     appointments from legacy scheduling systems, detect conflicts).
  2. The agent correctly assigns Tier B (manual span wrapping — no official
     EDOT instrumentor for aiohttp).
  3. The agent adds `with tracer.start_as_current_span(...)` with
     SpanKind.SERVER in each handler coroutine.
  4. The agent adds business enrichment attributes (patient.*, appointment.*,
     sync.*).
  5. The agent uses record_exception on error paths.
  6. The instrumented app starts and handles sync requests.
...
SVC         = "18-tier-b-aiohttp"
FIXTURE_DIR = os.path.join(os.path.dirname(__file__), "fixtures", "blank-aiohttp-appointments")
...
print(f"{'='*62}")
print(f"  Fixture: blank-aiohttp-appointments (no OTel)")
...
tmpdir = tempfile.mkdtemp(prefix="edot-autopilot-18-")
try:
    ...
    print("\nCode correctness checks (Tier B — manual aiohttp span wrapping):")
    ...
    check("Business enrichment attributes present",
          any(attr in app_content for attr in
              ["patient.", "appointment.", "sync.", "healthcare", "source_system"]),
          "no business enrichment attributes (patient.*, appointment.*, sync.*) found")
    ...
    if started:
        ...
        for patient in patients:
            r = http_lib.post(
                f"http://127.0.0.1:{PORT}/api/sync/appointments",
                json=patient, timeout=10,
            )
            check(
                f"POST /api/sync/appointments {patient['patient_id']} -> 200",
                r.status_code == 200,
                f"status={r.status_code} body={r.text[:200]}",
            )
            if r.status_code == 200:
                body = r.json()
                total = len(patient["appointments"])
                synced    = body.get("synced", 0)
                conflicts = body.get("conflicts", 0)
                check(
                    f"synced+conflicts==total for {patient['patient_id']}",
                    synced + conflicts == total,
                    f"synced={synced} conflicts={conflicts} total={total}",
                )

        # Bad request
        r_bad = http_lib.post(
            f"http://127.0.0.1:{PORT}/api/sync/appointments",
            json={"appointments": []}, timeout=2,
        )
        check("POST /api/sync/appointments missing patient_id -> 400",
