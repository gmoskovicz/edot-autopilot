#!/usr/bin/env python3
"""
EVAL TEST: Tier C — Stripe Payments Monkey-Patch (Real Agent Invocation)
=========================================================================
*** EVAL TEST — runs `claude -p` and costs ~$1-2 per execution ***

This test ACTUALLY runs `claude -p "Observe this project."` on a blank,
uninstrumented Stripe payment service. It does not assume or hardcode what
the agent will generate.

What this tests:
  1. The agent reads the blank app and understands it (Stripe SDK with no
     built-in OTel support — Tier C candidate)
  2. The agent correctly assigns Tier C (monkey-patching)
  3. The agent patches `stripe.Charge.create` (and optionally `stripe.Refund.create`)
     with CLIENT spans
  4. Business enrichment: payment.amount_usd, payment.currency,
     payment.customer_id, payment.charge_id, payment.status
  5. Error paths use record_exception + set_status(ERROR) for CardError
...
SVC = "04-eval-tier-c-stripe-monkey-patch"
FIXTURE_DIR = os.path.join(os.path.dirname(__file__), "fixtures", "blank-stripe-payments")
...
print(f"  Fixture: blank-stripe-payments (no OTel)")
...
tmpdir = tempfile.mkdtemp(prefix="edot-eval-04-")
try:
    ...
    check("stripe.Charge.create monkey-patched",
          ("stripe.Charge.create" in app_content
           and ("_orig" in app_content or "_original" in app_content
                or "= stripe.Charge.create" in app_content)),
          "stripe.Charge.create monkey-patch pattern not found in app.py")
    check("CLIENT span on stripe.Charge.create call",
    ...
    print("\nBusiness enrichment checks:")
    has_payment_enrichment = any(
        attr in app_content for attr in [
            "payment.amount_usd", "payment.currency", "payment.customer_id",
            "payment.charge_id", "payment.status", "payment.provider",
        ]
    )
    check("Business span attributes added (payment.amount_usd / payment.charge_id / etc.)",
          has_payment_enrichment,
          "no payment business enrichment attributes found in app.py")
