#!/usr/bin/env python3
"""
Eval test: ML Inference Platform (inference-gateway core service)
=================================================================
Runs `claude -p "Observe this project."` on the blank inference-gateway Flask
service (fixtures/blank-inference-gateway/) and verifies the agent adds correct
OTel instrumentation for ML workloads.

Services modeled: inference-gateway + result-cache + ab-testing + feature-store
                  + model-registry + serving-engine + explainability

Run:
    cd smoke-tests && python3 63-ml-inference/smoke.py
"""
...
SVC         = "63-ml-inference"
FIXTURE_DIR = os.path.join(os.path.dirname(__file__), "fixtures", "blank-inference-gateway")
...
print(f"{'='*62}")
print(f"  Fixture: blank-inference-gateway (ML platform, Flask, no OTel)")
print()
...
tmpdir = tempfile.mkdtemp(prefix="edot-autopilot-ml-")
try:
    ...
    subprocess.run(["git", "commit", "-m", "initial: blank ML inference gateway, no observability"],
    ...
    print("\nCode correctness checks (ML inference instrumentation):")
    ...
    if pip_result.returncode == 0:
        ...
        if started:
            try:
                ...
                check("POST /predict returns 200 or error",
                      r2.status in (200, 503), f"status={r2.status}")
            except urllib.error.HTTPError as e:
                check("POST /predict returns 200 or error",
                      e.code in (200, 503), f"HTTP {e.code}")
            except Exception as e:
                check("POST /predict returns 200 or error", False, str(e))
...
print(f"  Kibana → APM → inference-gateway (ml-inference)")
if failed:
    sys.exit(1)
